...
<body>
        ...
        <div class="row">
            <div class="col-sm-9 pl-0">
                <div class="cart-header">
                    <div class=" row">
                        <div class="col-6">
                            <form action="{{ asset('dashboard/orders/create') }}" method="post" id="formSearch">
                                @csrf
                            ...
                            @isset($key) 
                                <div>
                                    ...
                                    <a href="{{ asset('dashboard/orders/create') }}" style="margin-left: 20px; color: white"><i class="fas fa-times"></i></a>
                                </div>
                            @endisset
                        </div>
                    </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div>
                                                ...
                                                <form action="{{ asset('dashboard/orders/create/update') }}/{{ $key }}" method="post">
                                                    @csrf
                                        ...
                                        <td><a href='{{ asset('dashboard/orders/create/delete/') }}/{{ $key }}'><button class='btn btn-danger'><i class='fas fa-trash'></i></button></a></td>
                ...
                <div class="cart-item">
                    <div class="action-toolbar">
                        <div class="icon-s-f" id="sorts">
                            <a href="{{ asset('dashboard/orders/create/sort/asc') }}"><i class="fas fa-sort-numeric-up-alt"></i></a>
                            <a href="{{ asset('dashboard/orders/create/sort/desc') }}"><i class="fas fa-sort-numeric-down"></i></a>
                        </div>
                        {{ $product_s_c->links() }}
                    </div>
                    ...
                    <div class="row">
                        ...
                        @foreach($product_s_c as $key => $product)
                            <div class="col-6 col-sm-4 col-xl-2 cart">
                                ...
                                <form action="{{ asset("/dashboard/orders/create/add") }}/{{ $product->id_product }}" method="post" class="formAdd" id="form{{ $id++ }}">
                                    @csrf
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="check-out">
                            ...
                            <form action="{{ asset('dashboard/orders/add') }}" method="POST">
                                @csrf
                                        ...
                                        <div class="card card-body">
                                            <div class="wrap-actions">
                                                <div class="form-group">
                                                    ...
                                                    <label>Số điện thoại 
                                                        <input type="number" class="form-control" name="phone" placeholder="Nhập tên" autocomplete="off">
                                                    </label>
                                                    <label>Địa chỉ
                                                    ...
                                                    <label>Giao hàng
                                                        <select name="cod" class="form-control">
                                                            @foreach($cods as $key => $cod)
                                                                <option value="{{ $cod->id_shipping_unit }}">{{ $cod->name_shipping_unit }}</option>
                                                            @endforeach
                                                        </select>
                                                    </label>
                                    ...
                                    <div class="wraper-payment-content">
                                        <div class="payment-component" style="font-size: 17px;">
                                            <div class="payment-component-child ">
                                                ...
                                                <div class="form-group">
                                                    Tiền vận chuyển
                                                    <span class="sum-price money">{{ number_format(30000,0,',','.') }} đ</span>
                                                </div>
                                                ...
                                                @if($total > 0)
                                                    <div class="form-group" style="height: 30px;">
                                                        Thành tiền
                                                        @if(!isset($option))
                                                            <span class="sum-price total money">{{ number_format($total+30000,0,',','.') }} đ</span>
                                                            <input type="hidden" value="{{ $total + 30000 }}" id="total" name="total">
                                                        @else
                                                            @isset($option)
                                                                @if($option == 'percent')
                                                                    <span class="sum-price total money">{{ number_format(($total - ($total*$value/100)+30000) ,0,',','.') }} đ</span>
                                                                    <input type="hidden" value="{{ ($total - ($total*$value/100)+30000) }}" id="total" name="total">
                                                                @else
                                                                    <span class="sum-price total money">{{ number_format($total + 30000 - $value,0,',','.') }} đ</span>
                                                                    <input type="hidden" value="{{ $total + 30000 - $value }}" id="total" name="total">
                                                                @endif
                                                            @endisset
                                                        @endif
                                                    </div>
                                ...
                                @if( $qty < $stock || $stock !=0)
                                    <div class="checkout clear-cart" onclick="window.print()">
                                        <button class="btn-check" type="submit" style="outline: none">TẠO ĐƠN</button>
                                    </div>
                                @endif
    ...
    <div class="example-print">
        <div class=" row">
            ...
            <div class="col-6 text-left">
                <b>Khách hàng: </b> <span id="getCus">Khách lẻ</span><br>
            </div>
        </div><br>
        <div class="table">
    ...
    <div class="modal" id="discount">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <form action="{{ asset('dashboard/orders/create/discount') }}" method="post">
                    @csrf
                    <div class="modal-body">
                        <div class="option-sale" >
                            <div class="form-group">
                                ...
                                <input id="value" name="value" class="w-100" type="number" min="0" max="50" required placeholder="Nhập giá trị giảm">
                              </div>
                        </div>
                    </div>
                    <div class="modal-footer">
    ...
    <script>
        $(document).ready(function(){
            ...
            $('#getTotal').text($('.total').text());
            $('#Total').text($('.total').text());

            $('#search').keyup(function(){
                var key =  $('#search').val();
                $('#formSearch').attr('action', '{{ asset("/dashboard/orders/create/search") }}=' +key);
            });
